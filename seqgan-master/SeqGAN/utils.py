...
class GeneratorPretrainingGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(start, end):
            ...
            sentence = load_data(self.path)[idx]
            words = list(sentence)
...
class DiscriminatorGenerator(Sequence):
    ...
    def __init__(self, path_pos, path_neg, B, T=40, min_count=1, shuffle=True):
        ...
        with open(path_pos, 'rb') as f:
            data_tmp = pickle.load(f)
            self.n_data_pos = len(data_tmp)
        with open(path_neg, 'rb') as f:
            data_tmp_neg = pickle.load(f)
            self.n_data_neg = len(data_tmp_neg)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(start, end):
            ...
            if is_pos == 1:
                ...
                sentence = load_data(self.path_pos)[idx]
            elif is_pos == 0:
                # sentence = linecache.getline(self.path_neg, idx) # str
                sentence = load_data(self.path_neg)[idx]
